...
const CreatePostEditor = () => {
  ...
  const { channelId, parentHash, embedHash, text: defaultText } = useLocalSearchParams()
...
const CreatePostItem = memo(({ index }: { index: number }) => {
  ...
  const { updateText, activeIndex, removePost, setActiveIndex, posts } = useCreatePost()
    ...
    <>
      <Animated.View style={index === 0 ? {} : animatedStyle}>
        ...
        >
          ...
          <YStack gap="$2" marginTop="$1.5" width="100%" flexShrink={1} marginBottom="$4">
            <View>
              <TextInput
                ref={inputRef}
                value={post.text}
...
const CreatePostImageSelector = () => {
  ...
  return (
    ...
    >
      <View>
        <Image size={24} color={activeEmbedLimit === 0 ? '$mauve10' : '$mauve12'} />
      </View>
    </TouchableOpacity>
  )
}
...
const CreatePostBottomBar = () => {
  const { activePostLength, addPost, activePost, count, activeIndex } = useCreatePost()
...
const MentionSearch = () => {
  ...
  return (
    <View maxHeight="$15" borderTopWidth="$0.5" borderColor="$borderColor">
      {isLoading ? (
        <YStack alignItems="center" justifyContent="center" padding="$3" gap="$2">
          <Spinner />
...
const PostEmbedsDisplay = ({
  ...
  const isAllImages = embeds.every((content) => content.type?.startsWith('image/'))
