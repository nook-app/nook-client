...
import { ScrollView, Spinner, View, XStack } from 'tamagui'
    ...
    <View flexGrow={1}>
      ...
      >
        ...
        <View paddingHorizontal="$2">
          {urls.length > 1 && (
            <DotIndicator urls={urls} scrollX={scrollX} screenWidth={screenWidth} />
          )}
        </View>
        <IconButton onPress={handleDownload}>
          {isDownloading ? <Spinner /> : <Download size={24} color="$white" />}
        </IconButton>
      </XStack>
      <ScrollView
        ref={scrollViewRef}
      ...
      >
        {urls.map((url, index) => (
          <ReactNativeZoomableView
            key={index}
            ref={(el) => (zoomableViewRefs.current[index] = el)}
            minZoom={1}
            maxZoom={10}
