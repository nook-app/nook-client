...
export class PushService {
  ...
  async pushNotification(
    ...
    const messages = await this.formatPushNotification(users, notification);
    ...
    for (const ticket of tickets) {
      this.redis.setJson(`expoPushTicket:${ticket}`, ticket, 60 * 60 * 24);
    }
  }

  async formatPushNotification(
    ...
    switch (notification.type) {
      case NotificationType.LIKE: {
        ...
        const [user, cast] = await Promise.all([
          this.farcaster.getUser(n.sourceFid),
          this.farcaster.getCast(n.data.targetHash),
        ]);
        return users.map((u) => ({
          to: u.token,
      ...
      case NotificationType.REPLY: {
        ...
        const [user, cast] = await Promise.all([
          this.farcaster.getUser(n.sourceFid),
          this.farcaster.getCast(n.data.hash),
        ]);
        return users.map((u) => ({
          to: u.token,
      ...
      case NotificationType.FOLLOW: {
        ...
        const [user] = await Promise.all([this.farcaster.getUser(n.sourceFid)]);
        return users.map((u) => ({
          to: u.token,
